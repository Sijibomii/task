defmodule Websocket.Message do
  ...
  embedded_schema do
    ...
    field(:version, WebsocketHandler.Utils.Version)
    # reply messages only
    field(:errors, :map)
  end
  ...
  @type t :: %__MODULE__{
          operator: module(),
          payload: map(),
          reference: WebsocketHandler.Utils.UUID.t(),
          inbound_operator: String.t()
        }

  @spec changeset(%{String.t() => Websocket.SocketHandler.json()}, Websocket.SocketHandler.state()) :: Changeset.t()
  @doc """
  ...
  @operators Websocket.Message.Manifest.actions()
  ...
  defp cast_payload(changeset, state) do
    ...
    |> case do
      ...
      inner_changeset = %{valid?: false} ->
        errors = WebsocketHandler.Utils.Errors.changeset_errors(inner_changeset)
